...
function calculateComponentHeight(data) {
   ...
   if (data.componentType == "title") {
      ...
      if (data.abnormalFromValue !== 0 && data.abnormalToValue !== 0) {
         totalHeight += pageHeight / 19.4;
      }

      if (data.diffDiagnosis && data.diffDiagnosisImg) {
...
function generateHeaderImageComponent(url, height) {
   return `
      <div class="">
         ${
            !url
               ? `<div style="height: ${height}px;" class="bg-white w-full"></div>`
               : `<div style="height: ${height}px;" class="bg-white w-full"></div>`
         }
...
function generateTitleComponent(value, color, icon) {
   return `
       <div class="flex gap-2 items-center py-1">
          ...
          <h3 style="color:${color}" class="text-[0.875rem] py-1 font-semibold">${value}</h3>
       </div>`;
}
...
function generateTestReportHeaderComponent(tests) {
   ...
   html += `
       <div class= 'flex flex-col'>
          ...
          <div class="flex justify-between p-2 bg-[#cafbfb] text-[#023b3b] rounded-t-xl mb-2">
...
function generateEmptyComponent(height) {
   ...
   for (let i = 0; i < height; i++) {
      emptyRows += `<div class= 'flex  w-full pageBreak'> </div>`;
   }
...
function generateTestReportWithBarComponent(test, index) {
   ...
   html += `
                   ...
                   <div class="flex flex-col mb-1">
                      ...
                      <div class="flex flex-col bg-[#f4eaf5] rounded-b-md">
                         ...
                         <div class="flex justify-center">
                            <div
                               class=" w-[800px] h-[158px] flex justify-center"
      ...
      test.diffDiagnosis
         ? `
                         <div
                            class="flex flex-col gap-3 p-2 m-1 bg-white border-2 border-dotted border-violet-700 rounded-lg"
                         >

                         
                              <div class= "flex gap-2 ">
                              ${
                                 test.diffDiagnosisIcon
                                    ? `<img src="${test.diffDiagnosisIcon}"  style="width: 20px; height: 20px" />`
                                    : ""
                              }
                              <p class="text-sm">
                                 <span class="text-sm text-nowrap font-semibold">Differential Diagnosis:</span>
                                 ${
                                    test.diffDiagnosis ? test.diffDiagnosis : ""
                                 }</p>
                              </div>    
                            ...
                            ${
                              ...
                              <div class="">
                                 <img
                                    ...
                                    class="w-fit h-[180px] rounded-lg"
                                 />
                              </div>
                            `
                                  : ""
         ...
         : ``
                        ...
                        ${
                         ...
                         >
                                  <div class= "flex gap-2 ">
                              ${
                                 test.interpretationIcon
                                    ? `<img src="${test.interpretationIcon}"  style="width: 20px; height: 20px" />`
                                    : ""
                              }
                              <p class="text-sm">
                                 <span class="text-sm text-nowrap font-semibold">Interpretation:</span>
                                 ${
                                    test.interpretation
                                       ? test.interpretation
                                       : ""
...
<g id="SVGRepo_iconCarrier">
   ...
   <div class="test-report" data-index="${index}">
         ...
         <div class="flex flex-col bg-[#f4eaf5] rounded-b-md">
            ${
               test?.historical?.length > 1
                  ? `
                  <div class="flex justify-center">
                           <div class=" w-[800px] h-[158px] flex justify-center" id="chartContainer${
                              index - 0 + 1
                           }">
                           </div>
                  </div>
      ...
      test.diffDiagnosis
         ? `
               <div class="flex flex-col gap-3 p-2 m-1 bg-white border-2 border-dotted border-violet-700 rounded-lg">
                    <div class= "flex gap-2 ">
                              ${
                                 test.diffDiagnosisIcon
                                    ? `<img src="${test.diffDiagnosisIcon}"  style="width: 20px; height: 20px" />`
                  ...
                  ${
                     test?.diffDiagnosisImg
                        ? `
                              <div class="">
                                    <img src="${test?.diffDiagnosisImg}" class="w-fit h-[180px] rounded-lg" />
                              </div>
                  `
                        : ""
                  }
         ...
         : ``
            ...
            ${
               test.interpretation
                  ? `
                     <div class="flex flex-col gap-3 p-2 mx-2 my-2 bg-white border-2 border-dotted border-violet-700 rounded-lg">
                        <div class= "flex gap-2 ">
                              ${
                                 test.interpretationIcon
                                    ? `<img src="${test.interpretationIcon}"  style="width: 20px; height: 20px" />`
